...
function Animation(id, fps, urls) {
    ...
    // эта функция вызывается для каждого кадра анимации
    // this будет означать текущий кадр, но нам требуется доступ к объекту Animation
    // поэтому применяем замыкание current = this;
    ...
    // подчеркивание в имени метода указывает на то что он предназначен только
...
Animation.prototype.start = function () {
    ...
    if (!this.isLoaded) {
        ...
        // первый вызов для вывода очередной картинки
        this._nextFrame();

        // планировка последующих вызовов
