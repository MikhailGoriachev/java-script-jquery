...
class WorkerForm {
    ...
    initToHTML(idContainer = '') {
        ...
        $(`#frmPositionId${this.idNumber}`).html(
            getPositionList().reduce((acc, item) => (acc += `<option>${item}</option>`), '')
        );
        ...
        $(`#workerForm${this.idNumber}`).submit((e) => {
            // скрытие формы
            this.hide();
            ...
            this.worker.fullName = $(`#frmFullNameId${this.idNumber}`).val();
            this.worker.position = $(`#frmPositionId${this.idNumber}`).val();
            this.worker.gender = $(`#frmGenderId${this.idNumber}`).val() === 'женский';
            this.worker.yearOfEmployment = $(`#frmYearOfEmploymentId${this.idNumber}`).val();
            this.worker.salary = $(`#frmSalaryId${this.idNumber}`).val();
    ...
    show(isAdd = this.isAdd, worker = new Worker()) {
        ...
        $(`#frmHeader${this.idNumber}`).html(this.isAdd ? 'Добавление работника' : 'Редактирование работника');
        $(`#btnSubmitId${this.idNumber}`).val(this.isAdd ? 'Добавить' : 'Сохранить');
        $(`#workerForm${this.idNumber}`).css('display', 'block');

        // установка значений из объекта
        if (!this.isAdd) {
            $(`#frmFullNameId${this.idNumber}`).val(this.worker.fullName);
            $(`#frmPositionId${this.idNumber}`).val(this.worker.position);
            $(`#frmGenderId${this.idNumber}`).val(this.worker.gender ? 'женский' : 'мужской');
            $(`#frmYearOfEmploymentId${this.idNumber}`).val(this.worker.yearOfEmployment);
            $(`#frmSalaryId${this.idNumber}`).val(this.worker.salary);
        }
    }

    // скрытие формы
    hide() {
        $(`#workerForm${this.idNumber}`).css('display', 'none');
    }
