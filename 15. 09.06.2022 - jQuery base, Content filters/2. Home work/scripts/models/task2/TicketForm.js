...
class TicketForm {
    ...
    initToHTML(idContainer = '') {
        ...
        // заполнение списка
        $(`#frmDestinationId${this.idNumber}`).html(
            getCityList().reduce((acc, item) => (acc += `<option>${item}</option>`), '')
        );
        ...
        $(`#ticketForm${this.idNumber}`).submit((e) => {
            // скрытие формы
            this.hide();
            ...
            this.ticket.destination = $(`#frmDestinationId${this.idNumber}`).val();
            this.ticket.flightNumber = $(`#frmFlightNumberId${this.idNumber}`).val();
            this.ticket.fullName = $(`#frmFullNameId${this.idNumber}`).val();
            this.ticket.ticketCost = $(`#frmTicketCostId${this.idNumber}`).val();
    ...
    show(isAdd = this.isAdd, ticket = new Ticket()) {
        ...
        $(`#frmHeader${this.idNumber}`).html(this.isAdd ? 'Добавление заявки' : 'Редактирование заявки');
        $(`#btnSubmitId${this.idNumber}`).val(this.isAdd ? 'Добавить' : 'Сохранить');
        $(`#ticketForm${this.idNumber}`).css('display', 'block');

        // установка значений из объектов
        if (!this.isAdd) {
            $(`#frmDestinationId${this.idNumber}`).val(this.ticket.destination);
            $(`#frmFlightNumberId${this.idNumber}`).val(this.ticket.flightNumber);
            $(`#frmFullNameId${this.idNumber}`).val(this.ticket.fullName);
            $(`#frmTicketCostId${this.idNumber}`).val(this.ticket.ticketCost);
        }
    }

    // скрытие формы
    hide() {
        $(`#ticketForm${this.idNumber}`).css('display', 'none');
    }
