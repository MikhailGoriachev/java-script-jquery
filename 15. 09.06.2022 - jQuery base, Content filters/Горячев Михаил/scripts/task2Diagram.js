...
moduleTask2Diagram.createBarChart = function createBarChart(data, width, height, color) {
    ...
    let chart = $('<div></div>');
    chart.css({ width: width + 'px', height: height + 'px', position: 'relative' });
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        bar.html(`<h4 class="text-align-center">${data[i]}</h4>`);
        ...
        bar.css({ color: '#fff', borderRadius: '5px', boxSizing: 'border-box', padding: '0' });
...
window.addEventListener('load', function () {
    ...
    if (!moduleTask2Diagram.loadFromLocalStorage()) {
        $('#header').html('Нет данных для отображения');
        return;
    }
